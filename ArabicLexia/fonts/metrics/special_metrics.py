            ...
            if lookup.LookupType == 2 and hasattr(lookup, 'SubTable'):
                for subtable in lookup.SubTable:
                    if hasattr(subtable, 'PairSet'):
                        total_pairs += sum(len(ps.PairValueRecord) for ps in subtable.PairSet)
    return total_pairs

def calculate_special_metrics(analyzer):
    results = {}
    kerning_pairs_count = count_kerning_pairs(analyzer.font)
    results['kerning_quality'] = kerning_pairs_count # A single metric for all kerning
    ...
    space_width = None
    try:
        space_glyph_name = analyzer.cmap.get(32)
